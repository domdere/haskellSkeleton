...
#  CABAL_LIB_PATH
#  BINDINGS_DSL_INCLUDE
#  HSC2HS_FOUND
#  HSC2HS_VERSION
#  HSC2HS_VERSION_MAJOR
#  HSC2HS_VERSION_MINOR
...
FIND_PATH(CABAL_LIB_PATH config
    HINTS 
    $ENV{CABAL_LIB_PATH}
    PATHS 
    ~/.cabal
...
FILE (GLOB BINDINGS_DSL_PATHS "${CABAL_LIB_PATH}/lib/bindings-DSL-*/ghc-*/include")

FIND_PATH(BINDINGS_DSL_INCLUDE bindings.dsl.h
    PATHS
    ${BINDINGS_DSL_PATHS}
)

FIND_PROGRAM(HSC2HS_EXECUTABLE hsc2hs
  HINTS
  $ENV{HASKELL_DIR}
  #PATH_SUFFIXES bin
  PATHS
...
IF ( HASKELL_DEBUG )
    ...
    MESSAGE ( STATUS "Hsc2hs preprocessor: ${HSC2HS_EXECUTABLE}" )
ENDIF ( HASKELL_DEBUG )
...
FIND_PACKAGE_HANDLE_STANDARD_ARGS( HSC2HS DEFAULT_MSG HSC2HS_EXECUTABLE )
FIND_PACKAGE_HANDLE_STANDARD_ARGS( CABAL_LIB DEFAULT_MSG CABAL_LIB_PATH )
FIND_PACKAGE_HANDLE_STANDARD_ARGS( HSC2HS_BINDINGS_DSL DEFAULT_MSG BINDINGS_DSL_INCLUDE )

MARK_AS_ADVANCED( GHC_EXECUTABLE GHCI_EXECUTABLE CABAL_EXECUTABLE HADDOCK_EXECUTABLE HSC2HS )
...
MACRO (GET_HSC2HS_VERSION )
    EXECUTE_PROCESS (
        COMMAND "${HSC2HS_EXECUTABLE}" --version
        OUTPUT_VARIABLE  HSC2HS_VERSION__
        RESULT_VARIABLE  HSC2HS_VERSION_RESULT__
        ERROR_QUIET )

    IF ( NOT HSC2HS_VERSION_RESULT__)
        # if the result was False/0 it means there was no error
        # and we have the version output in HSC2HS_VERSION__
        # and we just have to parse it out.

        IF ( HASKELL_DEBUG )
            MESSAGE ( STATUS "${HSC2HS_EXECUTABLE} --version -> '${HSC2HS_VERSION__}'" )
        ENDIF ( HASKELL_DEBUG )
         

        STRING ( REGEX REPLACE "^hsc2hs version ([0-9]+)\\.[0-9]+.*" "\\1" HSC2HS_VERSION_MAJOR "${HSC2HS_VERSION__}" )

        STRING ( REGEX REPLACE "^hsc2hs version [0-9]+\\.([0-9]+).*" "\\1" HSC2HS_VERSION_MINOR "${HSC2HS_VERSION__}" )

        STRING ( REGEX REPLACE "^hsc2hs version ([\\.0-9^\\n]+).*" "\\1" HSC2HS_VERSION "${HSC2HS_VERSION__}" )

    ENDIF ( NOT HSC2HS_VERSION_RESULT__)

ENDMACRO (GET_HSC2HS_VERSION )
...
IF ( HSC2HS_FOUND )
    IF ( HASKELL_DEBUG )
        MESSAGE ( STATUS "Attempting to find version for hsc2hs." )
    ENDIF ( HASKELL_DEBUG )

    GET_HSC2HS_VERSION()

    IF ( HSC2HS_VERSION )
        MESSAGE ( STATUS "hsc2hs version determined to be: ${HSC2HS_VERSION}")
    ENDIF ( HSC2HS_VERSION )
