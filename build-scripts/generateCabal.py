...
def generateCabalString(
    ...
    cfgHasIncludeDirs = len(cfg['link-library-dirs']) > 0
    optionsHaveIncludeDirs = len(options.includeDirs) > 0

    if cfgHasIncludeDirs or optionsHaveIncludeDirs:
        if cfgHasIncludeDirs:
            includeDirs = includeDirs + ':' + ':'.join(cfg['link-library-dirs'])
    ...
    if cfgHasIncludeLibs or optionsHaveIncludeLibs:
        ...
        if cfgHasIncludeLibs:
            includeLibs = includeLibs + ':'.join(cfg['link-libraries'])

        if optionsHaveIncludeLibs:
            if cfgHasIncludeLibs:
                includeLibs = includeLibs + ':' 
            
            includeLibs = includeLibs + options.includes

    optionStr = '%s %s' % (includeDirs, includeLibs)
